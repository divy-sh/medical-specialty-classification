...
Original file is located at
    https://colab.research.google.com/drive/1HxfCNvI5Gasrnb_HPUGS7KBLX9y0gX51
"""
...
file_path = "/content/drive/MyDrive/mtsamples.csv"
...
from tensorflow.keras.layers import Embedding, LSTM, Dense, Bidirectional, Dropout, Attention, Input, GlobalAveragePooling1D
...
# Embedding Layer
embedding = Embedding(input_dim=max_words, output_dim=256, input_length=max_len)(input_layer)

# Bidirectional LSTM Layer (Fix: Keep return_sequences=True for Attention)
bilstm = Bidirectional(LSTM(128, return_sequences=True, dropout=0.2, recurrent_dropout=0.2))(embedding)

# Attention Mechanism (Fix: Use BiLSTM output)
attention = tf.keras.layers.Attention()([bilstm, bilstm])  # Self-attention over BiLSTM outputs

# Global Average Pooling (Fix: Reduce Attention output from (None, 50, 256) -> (None, 256))
pooled_output = GlobalAveragePooling1D()(attention)

# Fully Connected Layers
dense = Dense(128, activation='relu')(pooled_output)
dropout = Dropout(0.5)(dense)
output = Dense(len(label_encoder.classes_), activation='softmax')(dropout)  # Ensure correct output shape
...
# # Bidirectional LSTM Layer (Balanced version)
# bilstm = Bidirectional(LSTM(128, return_sequences=True, dropout=0.3, recurrent_dropout=0.3))(embedding)

# # Attention Mechanism (Apply over BiLSTM)
# attention = tf.keras.layers.Attention()([bilstm, bilstm])

# # Apply Global Pooling AFTER Attention (fixing issue)
# pooled_output = GlobalAveragePooling1D()(attention)

# # Fully Connected Layers (Keep it simple, no L2 regularization)
# dense = Dense(128, activation='relu')(pooled_output)
...
# # Compile Model (Increase learning rate)
# model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0005),
#               loss='sparse_categorical_crossentropy',
#               metrics=['accuracy'])
